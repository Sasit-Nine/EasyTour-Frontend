...
const CustomerManage = () => {
    ...
    const handleApprove = (documentId, isHistory = false) => {
        Modal.confirm({
            ...
            onOk: () => {
                APPROVE_MUTATION({
                    ...
                    refetch()
                    if (isHistory) {
                        setHistoryBookings(historyBookings.map(booking => 
                            booking.id === documentId ? { ...booking, status: "success" } : booking
                        ));
                    } else {
    ...
    const handleRejection = (documentId, isHistory = false) => {
        Modal.confirm({
            ...
            onOk: () => {
                APPROVE_MUTATION({
                    ...
                    if (isHistory) {
                        setHistoryBookings(historyBookings.map(booking => 
                            booking.id === documentId ? { ...booking, status: "failed" } : booking
                        ));
                    } else {
    ...
    useEffect(() => {
        if (data && data.bookings) {
            const transformedBookings = data.bookings.map((bk) => ({
                ...
                zipCode: bk.zipCode,
                Reciep: bk.payment?.stripe_receipt_url || "N/A", // Added null check here
                total_price: bk.total_price,
                quantity: bk.quantity,
                status: bk.booking_status,
                paymentStatus: bk.payment?.status_payment || "N/A", // Added null check here
                ...
                <div className="sm:flex-auto">
                    ...
                    <p className="mt-2 text-sm text-gray-700">
                        ...
                        <Input.Search 
                            ...
                            style={{ width: 500, marginLeft: '10px' }}
                        />
                    </p>
                </div>
                ...
                <Table
                    columns={[
                        ...
                        },
                        {
                            ...
                            key: "status",
                            render: (text) => (
                                <span className={text === "success" ? "text-green-600" : text === "failed" ? "text-red-600" : ""}>
                                    {text === "success" ? 'อนุมัติการจอง' : text === "failed" ? 'ปฏิเสธการจอง' : text}
                                </span>
                            )
                        },
                        {
                            ...
                            render: (text, record) => (
                                <div className="flex gap-3">
                                    <a onClick={() => handleApprove(record.id, true)} className="text-green-600 hover:text-green-900 cursor-pointer">
                                        อนุมัติ
                                    </a>
                                    <a onClick={() => handleRejection(record.id, true)} className="text-red-600 hover:text-red-900 cursor-pointer">
                                        ปฏิเสธ
                                    </a>
                                    <a onClick={() => showDetails(record)} className="text-[#F8644B] hover:text-[#F8644B] cursor-pointer">
                                        ดูเพิ่มเติม
                                    </a>
                                </div>
                            )
